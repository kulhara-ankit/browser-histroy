...
class HistorySearch extends Component {
  ...
  onDeleteHistory = id => {
    ...
    const updatedHistory = historyList.filter(eachItems => eachItems.id !== id)
  ...
  render() {
    ...
    const filteredSearchResults = historyList.filter(eachItem =>
        ...
        <div className="histories-search-container">
          {filteredSearchResults.length > 0 ? (
            <ul className="histories-list">
              {filteredSearchResults.map(eachHistory => (
                <HistoryItem
                  key={eachHistory.id}
